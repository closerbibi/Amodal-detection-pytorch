...
def normalize_bbox_3d_targets(roidb):
    ...
    for im_i in xrange(num_images):
        ...
        roidb[im_i]['bbox_3d_targets'] = \
                _compute_targets_3d(im_i, rois, boxes_3d, max_overlaps, max_classes)
    #return roidb
    ...
    for im_i in xrange(num_images):
        ...
        if len(np.where(np.isinf(targets))[0]) > 0:
            print im_i
            pdb.set_trace()
    ...
    for im_i in xrange(num_images):
        ...
        for cls in xrange(1, num_classes):
            ...
            if len(np.where(np.isnan(roidb[im_i]['bbox_3d_targets']))[0])>0:
                print im_i
                pdb.set_trace()
    # These values will be needed for making predictions
    # (the predicts will need to be unnormalized and uncentered)
    return roidb, means.ravel(), stds.ravel()
    
def _compute_targets_3d(im_i, rois, boxes_3d, overlaps, labels):
    ...
    #                                                 rois[gt_inds, :])
    ex_gt_overlaps = cython_bbox.bbox_overlaps(rois[ex_inds, :],
                                                     rois[gt_inds, :])
    ...
    # normalize target (check later for h and w)
