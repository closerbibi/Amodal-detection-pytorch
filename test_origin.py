...
#trained_model = 'models/saved_model3/faster_rcnn_%s.h5'%iter_choose
#trained_model = 'VGGnet_fast_rcnn_iter_100000.h5'

rand_seed = 1024

#save_name = 'HHA/%s'%iter_choose
save_name = '%s/%s'%(args.save, iter_choose)
max_per_image = 300
thresh = 0.05
vis = False
...
def test_net(name, net, imdb, max_per_image=300, thresh=0.05, vis=False):
    ...
    #_iter = trained_model.split('_')[-1].split('.')[0]
    _iter = iter_choose
    result_type = imdb_name.split('_')[-1]
    result_name = 'hha_%s'%_iter
    save_name = name.split('/')[0]
    imdb.evaluate_detections(all_boxes, save_name, result_name, output_dir)
    #_imd_._write_inriaresults_file(all_boxes, save_name, result_name)
    #cmd = "cd ./evaluation && matlab -nodesktop -r \"runall('{}', '{}', '{}'); exit;\" " \
    #                                        .format(result_type, save_name, result_name)
    #pdb.set_trace()
    #os.system(cmd)
